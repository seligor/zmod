...
3. Чем отличается работа  с экраном и без экрана
   ...
   В нашем принтере  128 мегабайт памяти, половину ест система, а 20 мегабайт съедает управление родным экраном. Если отключить родной экран, то мы экономим память. Но в этом случае начинают по другому работать встроенные средства печати. Старт печати, пауза, восстановление, отмена, окончание печати. И именно поэтому необходимо менять стартовый и конечный код. Также, при работе без экрана, принтер не выставляет Z-offset записанный на экране, и его необходимо передавать как параметр в [START_PRINT](https://github.com/ghzserg/zmod/wiki/Macros)
5. Я хочу удалить Zmod - мне придется все пере калибровать?
   
   Нет - все настройки сохраняются
7. Что такое альтернативная камера?
...
9. Я установил принтер, а ZMOD спрятал мою камеру! В Orca-FF я ее видел, а теперь она пропала!
   ...
   **Создайте новую камеру** c настройками по аналогии камеры "Пример_настройки_камеры":
   ...
   - your_IP - замените на IP адрес вашего принтера. На роутере поставьте *статический ip адрес для принтера* иначе он будет меняться, а камера  отваливаться.
10. Не могу камеру включить. нажимаю CAMERA_ON а мне в ответ "отключите камеру на экране принтера". там я в настройках её отключил, принтер перезагрузил.
...
11. Я установил последнюю версию, а разработчик говорит что надо обновиться.

    1. Удостоверьтесь, что вы поставили последнюю версию с флэшки
    2. Зайдите  в веб интерфейс. Настройки -> Обновления ПО -> Нажать "Проверить обновление"
