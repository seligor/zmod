...
11. Альтернативная реализаця работы камеры. Экономит память и позволяет изменить разрешение камеры.
...
2. Добавить КАМП после (имеет смысл только после  очистки сопла)
