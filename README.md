...
5. Включите  констроль [MD5]](https://github.com/ghzserg/zmod/wiki/Macros).  
6. Отправляейте файлы на  печать "Octo/Klipper". Родной протокол FF периодически передает битые файлы. В Orca. Щелкнуть по значку Wifi рядом с принтером
...
4. Я хочу удалить Zmod - мне придется все пере калибровать?
...
7. Не могу камеру включить. нажимаю CAMERA_ON а мне в ответ "отключите камеру на экране принтера". там я в настройках её отключил, принтер перезагрузил.
...
8. Я установил последнюю версию, а разработчик говорит что надо обновиться.
